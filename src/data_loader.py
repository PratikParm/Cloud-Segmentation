...
class CloudDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Making sure all labels are either 0 or 1
        label[label > 0.5] = 1.
        label[label <= 0.5] = 0.
...
def get_dataloaders(image_dir, label_dir, val_split, test_split, batch_size):
    ...
    # Create datasets
    train_dataset = CloudDataset(train_images, train_labels, transform_image=transform_img,
                                 transform_label=transform_label)
    val_dataset = CloudDataset(val_images, val_labels, transform_image=transform_img, transform_label=transform_label)
    test_dataset = CloudDataset(test_images, test_labels, transform_image=transform_img,
                                transform_label=transform_label)
